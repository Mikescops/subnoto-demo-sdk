...
function randomProTitle(): string {
  const parts = TITLE_PARTS.map(
    (arr) => arr[Math.floor(Math.random() * arr.length)],
  );
  return parts.join(" ");
}

export type MassUploadItem =
  | { envelopeUuid: string; documentUuid: string; title: string }
  | { error: string };

export type RunMassUploadResult =
  | { results: MassUploadItem[] }
  | { error: string };

export async function runMassUpload(options: {
  count: number;
  delayMs: number;
}): Promise<RunMassUploadResult> {
