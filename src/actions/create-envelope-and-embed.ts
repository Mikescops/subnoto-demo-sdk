...
const EMBED_BASE_URL =
  process.env.SUBNOTO_EMBED_BASE_URL ?? "https://app.subnoto.com";
...
function getClientAndWorkspace():
  ...
  if (!apiBaseUrl || !accessKey || !secretKey || !workspaceUuid) {
    return {
      error:
        "Missing env: SUBNOTO_BASE_URL, SUBNOTO_ACCESS_KEY, SUBNOTO_SECRET_KEY, WORKSPACE_UUID",
    };
  }
...
export type GetEnvelopeStatusResult =
  | { status: EnvelopeStatus }
  | { error: string };

export async function getEnvelopeStatus(
  envelopeUuid: string,
): Promise<GetEnvelopeStatusResult> {
  ...
  try {
    ...
    return {
      error:
        err instanceof Error ? err.message : "Failed to get envelope status",
    };
  }
}
...
export async function createEnvelopeAndEmbed(): Promise<CreateEnvelopeResult> {
  ...
  console.log(
    "[createEnvelopeAndEmbed] Env OK, baseUrl:",
    baseUrl,
    "workspace:",
    workspaceUuid,
  );
  ...
  if (!existsSync(pdfPath)) {
    ...
    return {
      error:
        "Missing assets/sample-multipage.pdf. Add it to the assets folder.",
    };
  }
  ...
  try {
    ...
    console.log(
      "[createEnvelopeAndEmbed] Step 1 OK — envelopeUuid:",
      envelopeUuid,
      "documentUuid:",
      documentUuid,
    );
      ...
      const msg =
        typeof addBlocksError === "object" && addBlocksError !== null
          ? ((addBlocksError as { error?: { message?: string } }).error
              ?.message ?? (addBlocksError as { message?: string }).message)
    ...
    console.log(
      "[createEnvelopeAndEmbed] Step 4: send (distributionMethod: none)…",
    );
    ...
    if (tokenError || !tokenData?.iframeToken) {
      console.error(
        "[createEnvelopeAndEmbed] Step 5 failed:",
        tokenError ?? "no iframeToken",
      );
      const msg =
        tokenError && typeof tokenError === "object" && tokenError !== null
          ? ((tokenError as { error?: { message?: string } }).error?.message ??
    ...
    const iframeUrl = `${EMBED_BASE_URL}${EMBED_SIGN_PATH}#t=${tokenData.iframeToken}`;
    console.log(
      "[createEnvelopeAndEmbed] Done — envelopeUuid:",
      envelopeUuid,
      "iframeUrl:",
      iframeUrl,
    );
  ...
  if (err instanceof SubnotoError) {
    if (/no session id/i.test(err.message)) {
      const hint =
        ...
        apiBaseUrl.includes("enclave.subnoto.com")
