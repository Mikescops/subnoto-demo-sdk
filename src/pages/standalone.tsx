      ...
      <div className="mx-auto max-w-2xl space-y-6">
        <div className="rounded-xl border border-[rgb(var(--color-border))] bg-[rgb(var(--color-surface-elevated))] p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm text-[rgb(var(--color-text-muted))]">
            This page demonstrates embedding the Subnoto signing experience in a minimal HTML page
            with no app shell. The iframe is loaded with a token in the URL hash (
            <code className="rounded bg-slate-100 px-1 text-xs">/embeds/sign#t=...</code>).
          </p>
          <p className="mt-2 text-sm text-[rgb(var(--color-text-muted))]">
            For a working example, create an envelope on the Create & Sign demo (<code className="rounded bg-slate-100 px-1 text-xs">/create-and-sign</code>), then use the same
            iframe URL in a static HTML file or another app. See{' '}
            <code className="rounded bg-slate-100 px-1 text-xs">index.html</code> in the project
            root for a minimal standalone template.
          </p>
        </div>
